...
def test_create_did(app_auth, client_auth_with_token):
...
def test_create_duplicated_did(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.carrier import Carrier
    from wazo_router_confd.models.carrier_trunk import CarrierTrunk
    from wazo_router_confd.models.domain import Domain
...
def test_get_did(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.carrier import Carrier
    from wazo_router_confd.models.carrier_trunk import CarrierTrunk
    from wazo_router_confd.models.domain import Domain
    ...
    session.commit()
    #
    response = client_auth_with_token.get("/1.0/dids/%s" % did.id)
    assert response.status_code == 200
    assert response.json() == {
        "id": did.id,
        "did_regex": r"^(\+?1)?(8(00|44|55|66|77|88)[2-9]\d{6})$",
...
def test_get_did_not_found(app_auth, client_auth_with_token):
    response = client_auth_with_token.get("/1.0/dids/1")
    assert response.status_code == 404


def test_get_dids(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.carrier import Carrier
    from wazo_router_confd.models.carrier_trunk import CarrierTrunk
    from wazo_router_confd.models.domain import Domain
    ...
    session.commit()
    #
    response = client_auth_with_token.get("/1.0/dids")
    assert response.status_code == 200
    assert response.json() == {
        "items": [
            {
...
def test_update_did(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.carrier import Carrier
    from wazo_router_confd.models.carrier_trunk import CarrierTrunk
    from wazo_router_confd.models.domain import Domain
...
def test_update_did_not_found(app_auth, client_auth_with_token):
    response = client_auth_with_token.put(
        "/1.0/dids/1",
        json={
            "did_regex": r"^(\+?1)?(8(00|44|55|66|77|88)[2-9]\d{6})$",
...
def test_delete_did(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.carrier import Carrier
    from wazo_router_confd.models.carrier_trunk import CarrierTrunk
    from wazo_router_confd.models.domain import Domain
    ...
    session.commit()
    #
    response = client_auth_with_token.delete("/1.0/dids/%s" % did.id)
    assert response.status_code == 200
    assert response.json() == {
        "id": did.id,
        "did_regex": r"^(\+?1)?(8(00|44|55|66|77|88)[2-9]\d{6})$",
...
def test_delete_did_not_found(app_auth, client_auth_with_token):
    response = client_auth_with_token.delete("/1.0/dids/1")
    assert response.status_code == 404
