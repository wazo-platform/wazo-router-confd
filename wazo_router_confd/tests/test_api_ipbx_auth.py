...
def test_create_ipbx(app_auth, client_auth_with_token):
...
def test_get_ipbx(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.domain import Domain
    from wazo_router_confd.models.tenant import Tenant
    from wazo_router_confd.models.ipbx import IPBX
    ...
    session.commit()
    #
    response = client_auth_with_token.get("/1.0/ipbxs/%s" % ipbx.id)
    assert response.status_code == 200
    assert response.json() == {
        "id": ipbx.id,
        "customer": 1,
...
def test_get_ipbx_not_found(app_auth, client_auth_with_token):
    response = client_auth_with_token.get("/1.0/ipbxs/1")
...
def test_update_ipbx_not_found(app_auth, client_auth_with_token):
    response = client_auth_with_token.put(
        "/1.0/ipbxs/1",
        json={
            'ip_fqdn': 'mypbx2.com',
...
def test_delete_ipbx(app_auth, client_auth_with_token):
    from wazo_router_confd.database import SessionLocal
    from wazo_router_confd.models.ipbx import IPBX
    from wazo_router_confd.models.domain import Domain
    from wazo_router_confd.models.tenant import Tenant
    ...
    session.commit()
    #
    response = client_auth_with_token.delete("/1.0/ipbxs/%s" % ipbx.id)
    assert response.status_code == 200
    assert response.json() == {
        "id": ipbx.id,
        "customer": 1,
...
def test_delete_ipbx_not_found(app_auth, client_auth_with_token):
    response = client_auth_with_token.delete("/1.0/ipbxs/1")
    assert response.status_code == 404
